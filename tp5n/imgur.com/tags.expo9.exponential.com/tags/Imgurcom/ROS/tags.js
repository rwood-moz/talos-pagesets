...
function expo9_ad() {
  ...
  t.displayAdURL = "httpdisabled://"+t.host+"/displayAd.js?dver=" + displayAdVersion + "&th=" + t.tagHash;
}
...
expo9_ad.prototype.showAd = function () {
  ...
  t.url = "httpdisabled://"+t.host+"/" + t.cmd + t.uparams + t.params + "&rnd=" + rnd;
...
function includeJScript(f) {
  void('<scr' + 'ipt type="text/javascript" src="' + f + '"><\/sc' + 'ript>');
}
...
function drawTags(t) {
  if (t.tagType == "iframe") {
     ...
     var hrefURL = "httpdisabled://"+t.host+"/i.click" + t.uparams + t.params;
...
function inspect (t) {
  for (var k in t) {
   if (typeof t[k] != 'function') {
      if (k == 'tagSrc') {
         void("<form><textarea wrap=on readonly name=start rows=13 cols=40>");
         void(t.tagSrc);
         void("</textarea></form>");
      } else {
        pr(t,k);
