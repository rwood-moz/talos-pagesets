if(typeof(linkwithin_is_running)==='undefined'){linkwithin_is_running=true;LW={v:35,u:'http://widget'+(Math.random()<0.55?(Math.random()<0.3?'':'2'):'3')+'.linkwithin.com/',hs:[],ts:[],ps:[],ws:[],wsl:[],dh:0};LW.m=function(f,a){var a2=[];for(var i=0,l=a.length;i!==l;++i){a2.push(f(a[i]));}
...
else if(document.getElementById){var oldOnScroll=window.onscroll;var oldOnResize=window.onresize;window.onloaddisabled=function(){oldOnScroll();LW.cl();}
...
LW.c('../www.linkwithin.com/get_custom_js');};LW.lws2=function(fhsc,fpsc){LW.fhsc=fhsc;LW.fpsc=fpsc;if(!LW.msie){try{LW.lws3()}
catch(e){}}
...
readyList=[];}}};function addLoadEvent(func){var oldonloaddisabled=window.onloaddisabled;if(typeof window.onloaddisabled!='function'){window.onloaddisabled=func;}else{window.onloaddisabled=function(){if(oldonloaddisabled){oldonloaddisabled();}
...
domReady();},false);}
if(browser.safari){var numStyles;(function(){if(isReady)return;if(document.readyState!="loaddisableded"&&document.readyState!="complete"){setTimeout(arguments.callee,0);return;}
