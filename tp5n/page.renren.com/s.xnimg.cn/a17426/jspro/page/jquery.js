...
E.addEventListener("loaddisabled",b.ready,false);
}else{
if(t.attachEvent){
t.attachEvent("onreadystatechange",u);
E.attachEvent("onloaddisabled",b.ready);
var j=false;
try{
...
}},beforeunloaddisabled:{setup:function(a,b,d){
if(c.isWindow(this)){
this.onbeforeunloaddisabled=d;
}
},teardown:function(a,b){
if(this.onbeforeunloaddisabled===b){
...
c.each("blur focus focusin focusout loaddisabled resize scroll unloaddisabled click dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup error".split(" "),function(a,b){
c.fn[b]=function(d,e){
if(e==null){
e=d;
d=null;
...
E.attachEvent&&!E.addEventListener&&c(E).bind("unloaddisabled",function(){
...
var mb=c.now(),nb=/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,ob=/^(?:select|textarea)/i,pb=/^(?:color|date|datetime|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i,qb=/^(?:GET|HEAD)$/,Ra=/\[\]$/,T=/\=\?(&|$)/,ja=/\?/,rb=/([?&])_=[^&]*/,sb=/^(\w+:)?\/\/([^\/?#]+)/,tb=/%20/g,ub=/#.*$/,Ha=c.fn.loaddisabled;
c.fn.extend({loaddisabled:function(a,b,d){
...
if(!d){
var C=false;
A.onloaddisabled=A.onreadystatechange=function(){
if(!C&&(!this.readyState||this.readyState==="loaddisableded"||this.readyState==="complete")){
C=true;
c.handleSuccess(b,w,e,f);
c.handleComplete(b,w,e,f);
A.onloaddisabled=A.onreadystatechange=null;
...
var J=false,w=b.xhr();
if(w){
b.username?void(h,b.url,b.async,b.username,b.password):void(h,b.url,b.async);
try{
...
jQuery.noConflict();
(function(){
var jsc=jQuery.now(),_4d1=/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,_4d2=/^(?:select|textarea)/i,_4d3=/^(?:color|date|datetime|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i,_4d4=/^(?:GET|HEAD)$/,_4d5=/\[\]$/,jsre=/\=\?(&|$)/,_4d7=/\?/,rts=/([?&])_=[^&]*/,rurl=/^(\w+:)?\/\/([^\/?#]+)/,r20=/%20/g,_4db=/#.*$/,_4dc=jQuery.fn.loaddisabled;
...
jQuery.ajax=function(_4df){
var s=jQuery.extend(true,{},jQuery.ajaxSettings,_4df),_4e1,_4e2,data,type=s.type.toUpperCase(),_4e5=_4d4.test(type);
s.url=s.url.replace(_4db,"");
s.context=_4df&&_4df.context!=null?_4df.context:s;
...
if(type==="GET"){
if(!jsre.test(s.url)){
s.url+=(_4d7.test(s.url)?"&":"?")+(s.jsonp||"callback")+"=?";
}
}else{
...
_4e1=s.jsonpCallback||("jsonp"+jsc++);
if(s.data){
s.data=(s.data+"").replace(jsre,"="+_4e1+"$1");
}
s.url=s.url.replace(jsre,"="+_4e1+"$1");
s.dataType="script";
var _4e6=window[_4e1];
window[_4e1]=function(tmp){
if(jQuery.isFunction(_4e6)){
_4e6(tmp);
}else{
window[_4e1]=undefined;
try{
delete window[_4e1];
}
catch(jsonpError){
...
data=tmp;
jQuery.handleSuccess(s,xhr,_4e2,data);
jQuery.handleComplete(s,xhr,_4e2,data);
if(head){
head.removeChild(_4ea);
}
};
}
...
if(s.cache===false&&_4e5){
var ts=jQuery.now();
var ret=s.url.replace(rts,"$1_="+ts);
s.url=ret+((ret===s.url)?(_4d7.test(s.url)?"&":"?")+"_="+ts:"");
}
if(s.data&&_4e5){
s.url+=(_4d7.test(s.url)?"&":"?")+s.data;
}
if(s.global&&jQuery.active++===0){
jQuery.event.trigger("ajaxStart");
}
var _4ed=rurl.exec(s.url),_4ee=_4ed&&(_4ed[1]&&_4ed[1].toLowerCase()!==location.protocol||_4ed[2].toLowerCase()!==location.host);
if(s.dataType==="script"&&type==="GET"&&_4ee){
var head=document.getElementsByTagName("head")[0]||document.documentElement;
var _4ea=document.createElement("script");
if(s.scriptCharset){
_4ea.charset=s.scriptCharset;
}
_4ea.src=s.url;
if(!_4e1){
var done=false;
_4ea.onloaddisabled=_4ea.onreadystatechange=function(){
if(!done&&(!this.readyState||this.readyState==="loaddisableded"||this.readyState==="complete")){
done=true;
jQuery.handleSuccess(s,xhr,_4e2,data);
jQuery.handleComplete(s,xhr,_4e2,data);
_4ea.onloaddisabled=_4ea.onreadystatechange=null;
if(head&&_4ea.parentNode){
head.removeChild(_4ea);
}
}
};
}
head.insertBefore(_4ea,head.firstChild);
return undefined;
}
var _4f0=jQuery("<a />").attr("href",_4df.url)[0].host;
if(!arguments[1]&&_4f0&&_4f0!=location.host){
jQuery("<iframe />").attr({"src":"httpdisabled://"+_4f0+"/ajaxproxy.htm","frameBorder":0,"width":0,"height":0}).appendTo("body").loaddisabled(function(){
var that=this;
var _4f2=_4df.success||function(){
};
_4df.success=function(){
_4f2.apply(this,arguments);
jQuery(that).remove();
};
jQuery.ajax(_4df,this.contentWindow);
});
...
try{
if((s.data!=null&&!_4e5)||(_4df&&_4df.contentType)){
...
var _4f4=xhr.onreadystatechange=function(_4f5){
if(!xhr||xhr.readyState===0||_4f5==="abort"){
if(!_4f3){
jQuery.handleComplete(s,xhr,_4e2,data);
}
_4f3=true;
...
if(!_4f3&&xhr&&(xhr.readyState===4||_4f5==="timeout")){
_4f3=true;
xhr.onreadystatechange=jQuery.noop;
_4e2=_4f5==="timeout"?"timeout":!jQuery.httpSuccess(xhr)?"error":s.ifModified&&jQuery.httpNotModified(xhr,s.url)?"notmodified":"success";
var _4f6;
if(_4e2==="success"){
try{
...
if(_4e2==="success"||_4e2==="notmodified"){
if(!_4e1){
jQuery.handleSuccess(s,xhr,_4e2,data);
}
}else{
jQuery.handleError(s,xhr,_4e2,_4f6);
}
if(!_4e1){
jQuery.handleComplete(s,xhr,_4e2,data);
}
...
xhr.abort=function(){
if(xhr){
Function.prototype.call.call(_4f7,xhr);
}
_4f4("abort");
};
}
...
try{
xhr.send(_4e5||s.data==null?null:s.data);
}
catch(sendError){
