...
return arg0;}
func=func.caller;}
return null;},hover:function(ele,classes){var el=HX.$(ele);HX.$V.bind(el,'mouseover',function(){var _this=this;HX.$E.addClass(_this,classes);});HX.$V.bind(el,'mouseout',function(){var _this=this;HX.$E.removeClass(_this,classes);});},ready:function(func,win,doc){var win=win||window;var doc=doc||document;var loaddisableded=false;var readyFunc=function(){if(loaddisableded)return;loaddisableded=true;func();};if(doc.addEventListener){this.bind(doc,'DOMContentLoaded',readyFunc);}else if(doc.attachEvent){this.bind(doc,'readystatechange',function(){if(doc.readyState=='complete'||doc.readyState=='loaddisableded')readyFunc();});if(doc.documentElement.doScroll&&typeof win.frameElement==='undefined'){var ieReadyFunc=function(){if(loaddisableded)return;try{doc.documentElement.doScroll('left');}catch(e){window.setTimeout(ieReadyFunc,0);return;}
readyFunc();};ieReadyFunc();}}
this.bind(win,'loaddisabled',readyFunc);}};HX.Cookie={get:function(name){var tmp,reg=new RegExp("(^| )"+name+"=([^;]*)(;|$)","gi");if((tmp=reg.exec(unescape(document.cookie))))
