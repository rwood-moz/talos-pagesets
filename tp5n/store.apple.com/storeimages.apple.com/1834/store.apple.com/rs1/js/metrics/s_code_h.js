...
if(!s_account) s_account="applestoreunspecified";
...
s.linkDownloaddisabledFileTypes="zip,wav,mp3,mp4,doc,pdf,xls,rss,dmg,sit,pkg,exe,xml,mov"
s.linkInternalFilters="javascript:,epp.apple.com,storeint.apple.com,store.apple.com"
...
function s_doPlugins(s) {
	...
	if(!s.eVar7) {
		...
		s.eVar7=s.getValOnce(s.eVar7,"s_aid");
	}

	// Campaign Pathing
	s.prop6=s.getQueryParam('cp')+": "+s.pageName;
	
	// Campaign Vendor and Affiliate Id
	if(!s.eVar10) {
		s.eVar10=s.getQueryParam('afid');
		s.eVar10=s.getValOnce(s.eVar10,"s_afc");
	}
	
	//Click Depth
	if(s.eVar7){
		if (s.eVar7.match(/CONFIGURE/)){
			s.eVar16=s.prop16="Configure Orders";
			s.events="event14";
		} else if (s.eVar7.match(/BUYNOW/)){
			s.eVar16=s.prop16="Buy Nows";
			s.events="event9";
		}
	}
...
/*
 * Plugin: setClickMapEmail 1.3 - sets ClickMap variables w/ q-string values
 */
...
+"var s=this;if(!s.fa){s.fa=new Object;var f=s.fa;f.ol=s.wd.onloaddisabled;s."
+"wd.onloaddisabled=s.faol;f.uc=s.useCommerce;f.vu=s.varUsed;f.vl=f.uc?s.even"
...
+".wd.onunloaddisabled;s.wd.onunloaddisabled=s.fasl;}return r;");
...
+"\"]=c;if(m._e)s.loaddisabledModule(n);if(s[n])for(j=0;j<m._l.length;j++)s_sv(m,s[n],m._l[j])}}}}}var e,o,t;try{o=voider;if(o&&o.s_gi){t=o.s_gi(\"'+s.un+'\");if(t)s_si(t)}}catch(e){}',1)}};s.c_d='';s"
...
+"f(dc=='d1')dc='112';else if(dc=='d2')dc='122';p=''}t1=un+'.'+dc+'.'+p+tb}rs='../metrics.gif;if"
...
+"=r;return ''}imn+='_'+s.rc[un];s.rc[un]++}im=s.wd[imn];if(!im)im=s.wd[imn]=new Image;im.s_l=0;im.onloaddisabled=new Function('e','this.s_l=1;var wd=window,s;if(wd.s_c_il){s=wd.s_c_il['+s._in+'];s.mrq(\"'+u"
...
+"';if(t&&h.indexOf(t)>=0)return 1;return 0};s.lt=function(h){var s=this,lft=s.linkDownloaddisabledFileTypes,lef=s.linkExternalFilters,lif=s.linkInternalFilters;lif=lif?lif:s.wd.location.hostname;h=h.toLower"
+"Case();if(s.trackDownloaddisabledLinks&&lft&&s.pt(lft,',','ltdf',h))return 'd';if(s.trackExternalLinks&&h.substring(0,1)!='#'&&(lef||lif)&&(!lef||s.pt(lef,',','ltef',h))&&(!lif||!s.pt(lif,',','ltef',h)))re"
...
+"_c_il['+s._in+'],r=true,b=s.eh(s.wd,\"onloaddisabled\"),i,o,oc;if(b)r=this[b](e);for(i=0;i<s.d.links.length;i++){o=s.d.links[i];oc=o.onclick?\"\"+o.onclick:\"\";if((oc.indexOf(\"s_gs(\")<0||oc.indexOf(\".s"
...
+".bc);else if(s.b&&s.b.addEventListener)s.b.addEventListener('click',s.bc,false);else s.eh(s.wd,'onloaddisabled',0,s.wdl)}};s.vs=function(x){var s=this,v=s.visitorSampling,g=s.visitorSamplingGroup,k='s_vsn_"
...
+"1]()}}m[f]=1;if(u)r=1}}return r};s.m_ll=function(){var s=this,g=s.m_dl,i,o;if(g)for(i=0;i<g.length;i++){o=g[i];if(o)s.loaddisabledModule(o.n,o.u,o.d,o.l,o.e,1);g[i]=0}};s.loaddisabledModule=function(n,u,d,l,e,ln){"
...
+"',b+'}');tcf=new Function('s','c','i','u','f1','f2','var e,o=0;try{o=s.d.createElement(\"script\");if(o){o.type=\"text/javascript\";'+(n?'o.id=i;o.defer=true;o.onloaddisabled=o.onreadystatechange=f1;o.f2=f"
...
+"Match,trackDownloaddisabledLinks,trackExternalLinks,trackInlineStats,linkLeaveQueryString,linkDownloaddisabledFileTypes,linkExternalFilters,linkInternalFilters,linkTrackVars,linkTrackEvents,linkNames,lnk,eo,_1_ref"
