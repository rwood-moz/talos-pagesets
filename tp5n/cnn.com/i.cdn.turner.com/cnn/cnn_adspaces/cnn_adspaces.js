...
function cnnad_renderAd(cnnad_url) {
	if(cnnad_enabled == true) {
		...
		void(" src=\""+cnnad_url+"&tile="+cnnad_getDynamicTileID(cnnad_url)+"&transactionID="+cnnad_getTransactionID()+"\"></scr");
		void("ipt>");
	}
}
...
function cnnad_createAdHelper (adId, cnnad_url, cnnad_height, cnnad_width, target) {
	if (cnnad_isBlocking(adId)) {
		...
		if (d) {
			...
			if (!target) {
				void('<iframe ALLOWTRANSPARENCY="true" hspace="0" vspace="0" marginHeight="0" marginWidth="0" src="' + cnnad_url + '" border="0" frameBorder="0" height="0" width="0" scrolling="no"  id="'+adId+'" style="position: absolute; top: -20px; left: -20px;" ></iframe>');
			} else {
...
function voidAd(cnnad_callid,cnnad_url) {
        if(cnnad_enabled == true) {
                void("<script id=\"" + cnnad_callid + "\" type=\"text/javascript\" onload=\"cnnSendData();\"");
                void(" src=\""+cnnad_url+"&tile="+cnnad_getDynamicTileID(cnnad_url)+"\"></scr");
                void("ipt>");
        }
}
...
function cnnad_startInterstitial(adId,cnnad_url,timeout) 
{
	...
	void('<div id="interstitial'+adId+'" class="interstitial" align="center"><iframe ALLOWTRANSPARENCY="true" hspace="0" vspace="0" marginHeight="0" marginWidth="0" src="'+adUrl+'" border="0" frameBorder="0" height="0" width="0" scrolling="no" id="'+adId+'"></iframe></div>');
