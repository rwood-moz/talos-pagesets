...
}D(window).bind("unloaddisabled",function(){var F=D.event._dpCache||[];
...
_tvdialogDisplayedJs=false;
if(D){window.location.reloaddisabled();
}}};
...
JASK.serp.searchhistory={queryHistorySwitch:function(){var G="";
var A="";
var H="";
var I=$j.cookie("qh");
if(I){G=I.split("-");
...
var J={path:"/",domain:"ask.com"};
$j.cookie("qh",A,J);
var B=$j("#first-sh");
if(B){B.hide();
}var C=$j("#next-sh");
if(C){C.hide();
}var F=$j("#deleteall-sh");
if(F){F.hide();
}var E=$j("#hide-sh");
if(E){E.show();
}var D=$j("#collapsibleLinkmenu-searchHistory");
if(D){D.hide();
}}},deleteQueryHistory:function(){var G="";
var A="";
var H=$j.cookie("qh");
G=H.split("-");
A=G[0]+"-";
...
}if((isNaN(P)||isNaN(P)||P>=K||P<=R)){if($j.trim(A.val())!=""&&$j.trim(N.val())!=""){var E="httpdisabled://www.realestate.com/"+N.val()+"/"+A.val()+"/"+Q.val()+"?esourceid=4412880&pricemin="+F+"&pricemax="+H+"&bedroomsmin="+J+"&bathroomsmin="+L+"&listingtypelist="+B;
$j("#adv_reform").attr("action",E);
return true;
}var E="httpdisabled://www.realestate.com/default.aspx?esourceid=4412880";
$j("#adv_reform").attr("action",E);
return true;
}else{if($j.trim(A.val())!=""&&$j.trim(N.val())!=""&&$j.trim(Q.val())!=""){var E="httpdisabled://www.realestate.com/"+N.val()+"/"+A.val()+"/"+P+"/"+Q.val()+"?esourceid=4412880&pricemin="+F+"&pricemax="+H+"&bedroomsmin="+J+"&bathroomsmin="+L+"&listingtypelist="+B;
$j("#adv_reform").attr("action",E);
return true;
}else{if(($j.trim(A.val())==""||$j.trim(N.val())=="")&&$j.trim(Q.val())!=""){var E="httpdisabled://www.realestate.com/"+P+"/"+Q.val()+"?esourceid=4412880&pricemin="+F+"&pricemax="+H+"&bedroomsmin="+J+"&bathroomsmin="+L+"&listingtypelist="+B;
$j("#adv_reform").attr("action",E);
return true;
}}}var E="httpdisabled://www.realestate.com/"+P+"/homes-for-sale.aspx?esourceid=4412880&pricemin="+F+"&pricemax="+H+"&bedroomsmin="+J+"&bathroomsmin="+L;
$j("#adv_reform").attr("action",E);
return true;
}};
JASK.serp.createAskDialog=function(){$j("#ask_question_dialog").dialog("destroy");
JASK.ask_dialog.loaddisabledAskDialog();
};
...
}$j("#soundspan").append($j("<embeddisabled>").attr({src:A,hidden:true,autostart:true,loop:false}));
return true;
};
...
if(D==JASK.currentTime.TRANSITION_TIME){window.location.reloaddisabled(true);
...
$j("#mini_me").loaddisabled(JASK.cfg.ldomain+"/ja-mini-me");
...
}$j.getScript(JASK.cfg.feedbackUrl+"/logout.json");
});
...
$j(document).bind("questionAsked",function(A,D){var C=$j.cookie("cp");
if(C){var B=C.split("|");
if(B[2]&&B[2]!=1){JASK.voidConfirm();
}}if(D!=0){$j("#mini_me_notification").html(($j.trim($j("#questionNotification").html())));
$j("#notification_link").attr("href",$j("#notification_link").attr("href").replace("000",D));
}});
...
$j("#asDlgWrap").loaddisabled(JASK.cfg.ldomain+$j("#advancedSearchLink").attr("href")+"?frcDlgVer=v14&q="+encodeURIComponent(A),function(){$j("#asDlg").dialog({autoOpen:false,height:600,width:750,modal:true,draggable:false,resizable:false,show:{effect:"fade",mode:"show",speed:300},hide:{effect:"fade",mode:"hide",speed:300}});
$j("#advancedSearchLink").click(function(){$j("#asDlg").dialog(void");
