...
function initSS() {
	...
	for(i in slideshow) {
		...
		if($(ssObj[i].slideshow).find(".tools .prev, .tools .prevNoFade").get(0)!=undefined){
			$(ssObj[i].slideshow).find(".tools .prev, .tools .prevNoFade").bind("click",{count:i},function(e){
				...
				if (ssObj[e.data.count].ssType == "image") {
					dcsMultiTrack('DCSext.embeddedSlideshowImage',$(ssObj[e.data.count].slideshow).find("> .ssImg:visible > img").attr("src"));
					trackGoogle();
		...
		if(ssObj[i].autoFunction == "randomFrame"){
			ssObj[i].timeoutId = setInterval(randomFrame,Number(ssObj[i].delay),i);
		}else if(ssObj[i].autoFunction == "nextFrame"){
			ssObj[i].timeoutId = setInterval(ssNext,Number(ssObj[i].delay),i);
		}else if(ssObj[i].autoFunction == "prevFrame"){
			ssObj[i].timeoutId = setInterval(ssPrev,Number(ssObj[i].delay),i);
		}
	}
}
