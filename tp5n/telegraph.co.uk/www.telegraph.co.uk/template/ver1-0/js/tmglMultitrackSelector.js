	...
	if ( $(".trackingItem").length > 0 ) {
		$(".trackingItem").each(function(){
			...
			if ( $(this).parents(".ssHide").length <= 0 && !$(this).hasClass(".hdn")){
				...
				dcsMultiTrack('DCSext.comTrackItem',$(this).comments(0)+'/'+$(this).comments(1)+'/'+$(this).comments(2)+'/'+$(this).comments(3),'WT.ad',$(this).comments(0));
			}
		});	
	}
}
...
function trackTabWidgetInitialAdImpressions() {
	...
	dcsRebuild();
	dcsMultiTrack("DCS.dcsuri","/widget/ads_displayed","WT.ad",adIds,"WT.dl","53","DCS.dcsref",DCS.dcssip+DCS.dcsuri);
...
$("a[href*='.pdf']").click(function(){
	dcsRebuild();
	dcsMultiTrack('DCSext.pdfDownloaddisabled',$(this).attr('href'),'WT.tx_u','1','DCS.dcsref',DCS.dcssip+DCS.dcsuri);
});
...
$("#diggWidget > .diggTabs > ul > .diggMPTab > a, #diggWidget > .diggTabs > ul > .diggUCTab > a, #diggWidget > .diggTabs > ul > .diggRTab > a").click(function(){
	dcsRebuild();
	dcsMultiTrack('DCSext.widgetId','Digg Widget','DCSext.tabClick',$(this).text(),'WT.tx_u','1','DCS.dcsref',DCS.dcssip+DCS.dcsuri);
});
// Story Clicks - refer to digg widget code
...
$(".configTabs .tabs a").click(function() {
	...
	dcsRebuild();
	dcsMultiTrack("DCS.dcsuri","/widget/tab_changed","WT.ad",adIds,"WT.dl","53","DCS.dcsref",DCS.dcssip+DCS.dcsuri);
...
$("#tmglPrimaryNav li a").click(function() {
	getNavPos($(this).attr("href"),"primary");
});
$("#tmglSecondNav li a").click(function() { 
	getNavPos($(this).attr("href"),'secondary');
});
$("#tmglThirdNav li a").click(function() { 
	getNavPos($(this).attr("href"),'tertiary');
});
$("#tmglHotTopics li a").click(function() { 
	getNavPos($(this).attr("href"),'hottopics');
});
...
function getNavPos(href,nav) {
	if((href.indexOf("httpdisabled://www")==0)||(href.indexOf("/")==0)){
