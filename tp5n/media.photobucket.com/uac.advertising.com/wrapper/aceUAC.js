...
var w=window,site=''
  ...
  if (obj.exchmap!='0'){
   if (w.addEventListener)w.addEventListener("loaddisabled",function(event) {ACErtbCheck(obj.extra);},false)
   else if (w.attachEvent)w.attachEvent("onloaddisabled",function() {ACErtbCheck(obj.extra);})
  }
  var nmVal={
  media: 'mnum',
  leadback: 'betr',
  ...
  var pt=obj.poptype,pu=obj.poponunloaddisabled

  if (gl!=1){
    if (pt||pu){ht=ACE3PopHost;gl=1}
    if (se>-1){
      ht="httpdisabledsdisabled://secure.ace"+adv
      gl=1
    }
    if (obj.host){var v=obj.host;ht=(v.indexOf('http')==-1?"httpdisabled://"+v:v);gl=1}
  }

  if (gl!=1){
    if (obj.Class==0){
             ht="httpdisabled://cs.ace"+adv
             ot+="/agv=1"
             gl=1
    }
    else if (obj.Class==2){
              ht="httpdisabled://cte.ace.beta"+adv
              ot+="/cte=1"
              gl=1
    }
    else if (obj.Class==3){
              ht="httpdisabled://cte.ace"+adv
              ot+="/cte=1"
              gl=1
    }
    else if (obj.Class==4){
	      ht="httpdisabled://beta.cs.ace"+adv
	      ot+="/agv=1"
	      gl=1
     }
  }
  ...
  if (gl!=1){
      ...
      if (window.ACE3NewHost&&ACE3NewHost[site]!=null){
        ...
        if (rand <= ACE3GlobalHostPercent){
           ...
           ht="httpdisabled://ace"+adv
        }
      }
      else {
         if (obj.region==2)ht="httpdisabled://r2.ace"+adv
         else if (obj.region==3)ht="httpdisabled://sr-r3.ace"+adv
         else ht="httpdisabled://r1-ads.ace"+adv
      }
 }
...
win=void(n[0],'win',"width="+n[1]+",height="+n[2]+",status=0,location=0")
f=n[3]
}
...
win=void(m,'win',"width="+w+",height="+h+",status=0,location=0")
}
if (p){
if (f!='0'){
window.focus()
