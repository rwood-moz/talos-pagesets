...
function makeFlashObj(_swfURL_,_flashID_,_width_,_height_,_wmode_,_flashVars_,_bgColor_,_allowFullScreen_,_menu_){
	...
	var arr_obj = [];
	arr_obj.push('<objectdisabled width="'+_width_+'" height="'+_height_+'" id="'+_flashID_+'" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="httpdisabled://fpdownloaddisabled.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" >');
	...
	arr_obj.push('<embeddisabled src="'+_swfURL_+'" quality="high" wmode="'+_wmode_+'" menu= "'+_menu_ +'" FlashVars="'+_flashVars_+'" bgcolor="'+_bgColor_+'" width="'+_width_+'" height="'+_height_+'" name="'+_flashID_+'" allowFullScreen="'+_allowFullScreen_+'" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="httpdisabled://www.macromedia.com/go/getflashplayer"  />');
	arr_obj.push('</object>');
...
function showFlash(_swfURL_,_flashID_,_width_,_height_,_wmode_,_flashVars_,_bgColor_,_allowFullScreen_){
	void(makeFlashObj(_swfURL_,_flashID_,_width_,_height_,_wmode_,_flashVars_,_bgColor_,_allowFullScreen_));
}
...
function flashExternalCleanup() {
	try {
	__flash_unloaddisabledHandler = function(){
	...
	if (window.onunloaddisabled != __flash_unloaddisabledHandler){
		__flash_savedUnloaddisabledHandler = window.onunloaddisabled;
		window.onunloaddisabled = __flash_unloaddisabledHandler;
	}
	} catch (e) {}
}
window.onbeforeunloaddisabled=flashExternalCleanup;

var isshift = false;
var loaddisabledFlash = false;
...
function savedMe(objs) {
	if (objs.checked == true) {
		  $('chk_me').checked = true;
		  $('chk_me2').checked = true;
		  $('chk_me3').checked = true;
		  $('nvme').value = "1";
	} else {	  
		  $('chk_me').checked = false;
		  $('chk_me2').checked = false;
		  $('chk_me3').checked = false;
		  $('nvme').value = "0";
	}
}
...
function getKeyByRuntimeInclude() {
	try {
		...
		keyjs.src = 'http://static.nid.naver.com/enclogin/keys_js.nhn';
		document.getElementsByTagName('head')[0].appendChild(keyjs);
	} catch (e) {
		showErrorDiv(4);
	}
}
function setMeCheckCookie(vals) {
	var today = new Date();
	var expire = new Date(today.getTime() + 60 * 60 * 24 * 365 * 1000);
	var curCookie = "nid_me=" + escape(vals) + "; expires="
			+ expire.toGMTString() + "; path=/; domain=.nid.naver.com;";
	document.cookie = curCookie;
}
function confirm_submit() {
	if ($('enctp').value == 1) {
		...
		if ($('chk_me').checked == true) {
			$('nvme').value = "1";
			$('url').value = "httpdisabled://me.naver.com";
			setMeCheckCookie(1);
		} else {
			$('nvme').value = "0";
			setMeCheckCookie(0);
		...
		if ($('chk_me').checked == true) {
			try {parent.clickcr($('lv1_submit'), 'log_off.melogin', '', '', e);} catch (e) {}
		} else {
//			try {parent.clickcr($('lv1_submit'), 'log_off.login', '', '', e);} catch (e) {}
		}
		...
		if (level3_frame.thirdAction()) {
			...
			if ($('chk_me').checked == true) {
				setMeCheckCookie(1);
				try {parent.clickcr($('lv3_submit'), 'log_off.melogin', '', '', e);} catch (e) {}
			} else {
				setMeCheckCookie(0);
//				try {parent.clickcr($('lv3_submit'), 'log_off.login', '', '', e);} catch (e) {}
			}
			return true;
		} else {
			return false;
...
function loaddisabledComplete() {
	loaddisabledFlash = true;
}
...
function login(keyData) {
	...
	if ($('chk_me2').checked == true) {
		$('nvme').value = "1";
		$('url').value = "httpdisabled://me.naver.com";
		setMeCheckCookie(1);
		try {parent.clickcr($('lv2_submit'), 'log_off.melogin', '', '', e);} catch (e) {}
	} else {
		$('nvme').value = "0";
		setMeCheckCookie(0);
//		try {parent.clickcr($('lv2_submit'), 'log_off.login', '', '', e);} catch (e) {}
	}
	$('frmNIDLogin').submit();
}
...
function onActiveXInst() {
	...
	$('level3_frame').src = "/loginv2/waitInst.nhn?20090507";
}
function needDelay() {
...
function cookie_init2() {
	...
	nvme = getCookie("nid_me")? getCookie("nid_me") : 0;
	if (nvme == 1) {
		$('chk_me').checked = true;
		$('chk_me2').checked = true;
		$('chk_me3').checked = true;
	...
	try {
		//idSetting();
		if ( navigator.userAgent.indexOf("Mobile Safari") == -1 && (navigator.userAgent.indexOf(";01") != -1 || navigator.userAgent.indexOf("POLARIS 6.100;em1.0;lgtelecom") != -1) ) {
			$('uid').style.left='14px';
			$('uid').style.top='-47px';
			$('upw').style.left='14px';
			$('upw').style.top='-47px';
...
function useLevel(levelno) {
	...
	if (chglevel == 3) {
		$('upw').value = "";
		$('level3_frame').src = "/loginv2/activexme.nhn";
	} else if (chglevel == 2) {
		$('upw').value = "";
		/* flash loaddisableding problem delay 500 ms */
		clear3rdLevel();
		...
		try {
				$('flasharea').innerHTML = showFlashagain("/loginv2/commonLoginF.swf", "flashlogin", 105, 49, "window", "null", "#f7f7f7", "false");
				loaddisabledFlash = false;
			} catch(e) {}
		}
		if (!loaddisabledFlash) {
...
function setLevelImg(lv) {
	if (lv==1) {
		...
		$('lv1_img').src='../btn_security1_on.gif';
		$('lv2_img').src='../btn_security1_on.gif';
		$('lv3_img').src='../btn_security1_on.gif';
	} else if (lv==2) {
		$('slide_btn1').className ='level1';
		$('slide_btn2').className ='level2 level_on';
		$('slide_btn3').className ='level3';
		$('lv1_img').src='../btn_security1_off.gif';
		$('lv2_img').src='../btn_security2_on.gif';
		$('lv3_img').src='../btn_security3_off.gif';
    } else if (lv==3) {
		$('slide_btn1').className ='level1';
		$('slide_btn2').className ='level2';
		$('slide_btn3').className ='level3 level_on';
		$('lv1_img').src='../btn_security1_off.gif';
		$('lv2_img').src='../btn_security2_off.gif';
		$('lv3_img').src='../btn_security3_on.gif';
    } else {
		$('slide_btn1').className ='level1';
		$('slide_btn2').className ='level2 level_on';
		$('slide_btn3').className ='level3';
...
function checkAd(flags) {
	...
	if (!onError) {
		if ((flags == '1' || flags == 1)
			...
			$('level_img1').src = '../static.naver.com/green/v2/img/h_security' + initEnc + '_txt.gif';
		} else {
			addshow=false;
			if (swapLevel == 2) {
				if (navigator.userAgent.indexOf("Opera") != -1 || navigator.userAgent.indexOf("Safari") != -1 || (navigator.userAgent.indexOf("Safari") != -1 && navigator.platform.indexOf("Mac") != -1 )) {
			...
			$('level_img1').src = '../static.naver.com/green/v2/img/h_security1_txt.gif';
			swapLevel = -1;
			hideElement("m1");
			try {
				//$('upw').disabled=false;
...
function setIdTextField() {
	...
	idHeight = "20";
	idMaxChars = "25";
	idTextDefault = "httpdisabled://static.naver.com/common/login/bg_login_id_main_6.gif";
	idTextFont = "돋움";
	idBoldTag = ""; // true
	idTextColor = "444444";
	idTextSize = "12";
...
function setPwTextField() {
	...
	pwHeight = "20";
	pwMaxChars = "16";
	pwTextDefault = "httpdisabled://static.naver.com/common/login/bg_login_pw_main_6.gif";
	pwTextFont = "돋움";
	pwBoldTag = ""; // true
	pwTextColor = "444444";
	pwTextSize = "12";
