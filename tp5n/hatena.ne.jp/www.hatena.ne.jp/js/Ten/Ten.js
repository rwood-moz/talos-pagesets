...
Ten.XHR = new Ten.Class({
    initialize: function(uri,opts,obj,callPropertyName) {
        ...
        this.loaddisabled(uri, opts.data);
    },
    ...
    loaddisabled: function(url, params) {
...
Ten.DOM = new Ten.Class({
    ...
    addObserver: function() {
        var c = Ten.DOM;
        if (c.observer || c.loaddisableded) return;
        c.observer = new Ten.Observer(window,'onloaddisabled',c,'finishLoad');
        ...
        if (window.opera || ua.indexOf('FIREFOX') >= 0) {
            ...
            (function() {
                ...
                try {
                    if (document.readyState != 'loaddisableded' &&
    ...
    finishLoad: function() {
        ...
        if (!c.loaddisableded) {
            ...
            c.dispatchEvent('onloaddisabled'); // for backward compatibility
            c.loaddisableded = true;
            c.observer.stop();
            c.observer = null;
        }
    },
...
Ten.Deferred = (function () {
    ...
    Deferred.next_faster_way_readystatechange = ((typeof window === 'object') && (location.protocol == "httpdisabled:") && !window.opera && /\bMSIE\b/.test(navigator.userAgent)) && function (fun) {
        var d = new Deferred();
    ...
    Deferred.next_faster_way_Image = ((typeof window === 'object') && (typeof(Image) != "undefined") && !window.opera && document.addEventListener) && function (fun) {
        ...
        img.addEventListener("loaddisabled", handler, false);
        img.addEventListener("error", handler, false);
        d.canceller = function () {
            img.removeEventListener("loaddisabled", handler, false);
