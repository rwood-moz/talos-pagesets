...
var fromTop = null;
// Default fÃ¼r den Background der Lightbox, kann in HTML-Farbwerte geÃ¤ndert werden. Per default erstmal leer
...
de.bild.qgtv.ShowLightbox = function(source, type, params, background, draggable, delay){
    ...
    jQuery.ajax({
        ...
        type: type,
        beforeSend: function(){
            de.bild.qgtv.loaddisablederIcon('.lightbox')
        },
        complete: function(){
...
de.bild.qgtv.ActivateLightbox = function(){
 ...
 'background:' + bg + ';"></div>');
    ...
    // esc-Taste gedrÃ¼ckt? Dann schlieÃŸe die Lightbox
...
/** LaderÃ¤dchen-SpaÃŸ */
de.bild.qgtv.loaddisablederIcon = function(element){
    //setTimeout(function(){
...
/** Funktion zur Aninmation des DSM Ergebnisses. ZÃ¤hlt das aktuelle Ergebnis um 1 hoch
 *
 * @param id Die Id des Votings, nicht nÃ¶tig, wenn nur ein Voting auf der Seite verbaut ist.
...
de.bild.qgtv.count = function(timeout, id){
    ...
    setTimeout(function(){
        ...
        //FÃ¼r den Fall, dass Votings IDs bekommen, muss diese Zeile auskommentiert, bzw bearbeitet werden
        if (id != null) {
            if (id.charAt(0) != "#") {
                id = "#" + id;
            }
        ...
        var votes = "";
        //Diese Variable dient zur ÃœberprÃ¼fung, ob sich die Anzahl der Stellen nach dem Abstimmen verÃ¤ndert hat
        var votes_overflow = "";
        //Variable, die angibt, ob sich die Anzahl der Stellen verÃ¤ndert hat
        var overflow = "";
        ...
        //in diesem String werden die fÃ¼hrenden Nullen einer Zahl gespeichert
        var leading_zero = "";
        //gibt an, ob man sich noch am Anfang der Zahl befindet
        var leading = 1;
      
        //Anhand der Klassennamen wird hier der alte Ergebnisstand ermittelt. DafÃ¼r werden alle <span> Attribute des DSM Ergebnisses durchgegangen
        //und jeweils das letzte Zeichen der Klasse ausgelesen und zu einer Zahl zusammengefÃ¼gt
        id.children('span').each(function(index, value){
            ...
            //Handling von Zahlen, die eine fÃ¼hrende Null besitzenz.B: 003
            if (leading && span == "0") {
                leading_zero += "0";
            }
            else {
            ...
            votes += (span);
            //um spÃ¤ter zu prÃ¼fen, ob eine neue Stelle hinzukommt, gibt es eine PrÃ¼fvariable
            votes_overflow += (span);
        });
      
        //Votingergebnis hochzÃ¤hlen
        votes++;
        votes = votes.toString();
        // FÃ¼hrende Nulle vorne ranhÃ¤ngen. Ausnahme bietet der Fall, dass man von 0 auf 1 zÃ¤hlt, da es dabei keinen Ãœbertrag gibt aber trotzdem eine fÃ¼hrende
        // 0 verschwindet
        if (votes != "1") {
        ...
        //ÃœberprÃ¼fung, ob durch die Addition eine Stelle hinzukommt. z.B: 9+1=10
        if (votes.length != votes_overflow.length) {
            overflow = 1;
            overflow_number = votes.charAt(votes.length - 1);
            if (leading_zero.length != 0) {
                //votes und span Ã¼m je eine leading-zero kÃ¼rzen, da es einen Ãœbertrag gibt
        ...
        //Vergleich, welche Zahl sich geÃ¤ndert hat und hinzufÃ¼gen eines Ãœberblendungs-Span-Tags
        id.children('span').each(function(index, value){
            var span = jQuery(value).attr('class');
        ...
        //Wenn eine neue Stelle hinzukommt, wird am Ende der Zahl der Ãœbertrag hinzugefÃ¼gt und gleich animiert
        if (overflow) {
        ...
        //Ãœberlagern der sich geÃ¤nderten Zahlen. Die neue(n) Zahl(en) schieben sich vor die alten. Im Hintergrund Ã¤ndern die alten Zahlen
        // ihre Klassen auf die neue Zahl und die fÃ¼r die Anmimation verwendeten Zahlen werden entfernt
 ...
 * @param ms Datum in ms, an dem der Countdown ablÃ¤uft
...
de.bild.qgtv.timer = function(ms, elem){
    jQuery(document).ready(function(){
        ...
        if (jQuery(elem).length != 0) {
            ...
            //unter alle Zahlen, die sich verÃ¤ndern ein Layer legen fÃ¼r die Animation
 ...
 * Checkbox-FunktionalitÃ¤t auch auf p-Tags
 * Radiobutton-FunktionalitÃ¤t auch auf p-Tags
 -------------------------------------------------------------------------------------------*/
//-----FUNKTIONEN---------------------------------------------------------------------------
//-----FUNKTION:OnClick auf Listenfeld
QGTV.checklistClick = function(){
    //"this" ist das aktuelle <p>-Tag aus dem HTML
    //das zum <p>-Tag gehÃ¶rende Input-Objekt
...
QGTV.checkboxClasse = function(obj){
    ...
    //das zum <p>-Tag gehÃ¶rende Label-Objekt bekommt neuen Klassennamen
