...
var jQuery = function( selector, context ) {
	...
	// Map over jQuery in case of void
	_jQuery = window.jQuery,

	// Map over the $ in case of void
	_$ = window.$,
...
jQuery.extend({
	...
	ready: function() {
		// Make sure that the DOM is not already loaddisableded
		if ( !jQuery.isReady ) {
	...
	bindReady: function() {
		...
		if ( document.addEventListener ) {
			...
			// A fallback to window.onloaddisabled, that will always work
			window.addEventListener( "loaddisabled", jQuery.ready, false );

		// If IE event model is used
		} else if ( document.attachEvent ) {
			// ensure firing before onloaddisabled,
	...
	add: function( elem, types, handler, data ) {
		...
		if ( !eventHandle ) {
			elemData.handle = eventHandle = function() {
				...
				// an event is called after a page has unloaddisableded
...
// Window isn't included so as not to unbind existing unloaddisabled events
// More info:
	...
	window.attachEvent("onunloaddisabled", function() {
		for ( var id in jQuery.cache ) {
			if ( jQuery.cache[ id ].handle ) {
				// Try/Catch is to handle iframes being unloaddisableded, see #4280
				try {
...
var isXML = function(elem){
	...
	// (such as loaddisableding iframes in IE - #4833) 
...
function buildFragment( args, nodes, scripts ) {
	...
	// IE 6 doesn't like it when you put <objectdisabled> or <embeddisabled> elements in a fragment
...
var jsc = now(),
	...
	// Keep a copy of the old loaddisabled method
...
jQuery.extend({
	...
	ajax: function( origSettings ) {
		...
		if ( s.dataType === "json" && (s.data && jsre.test(s.data) || jsre.test(s.url)) ) {
			...
			// Handle JSONP-style loaddisableding
		...
		// and trying to loaddisabled JSON or Script with a GET
